...
def get_championship_table(teams):
    ...
    championship_table._set_field_names(["Место", "Название", "Побед", "Поражений", "Ничья", "Забито", "Пропущено", "Очки"])

    # сортировка результатов чемпионала и присвоение мест
    teams.sort(key=operator.attrgetter('score'), reverse=True)
    for team in teams:
        championship_table.add_row([team.place, team.name, team.number_of_wins, team.number_of_loses, team.number_of_dead_heats, team.scored_goals, team.goals_agains, team.score])
...
def get_information_about_match(team_name_1, team_name_2, results):
    ...
    return(('%d:%d' % (score_1, score_2)) if 'score_1' in locals() and 'score_2' in locals() else 'Такого матча не существует')
...
def get_match(team_1, team_2):
    ...
    :param return: словарь вида: (название команды 1, название команды 2): (cчёт 1, счёт 2)
    '''
...
if __name__ == '__main__':
    ...
    while(True):
        ...
        if answer.lower() in ['y', 'yes', 'д', 'да']:
            try:
                team_1, team_2 = input('Введите названия команд через запятую и пробел: ').split(', ')
            except ValueError:
                ...
                print('\t', get_information_about_match(team_1, team_2, championship_results))
